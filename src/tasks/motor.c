...
static void apply_motor_state(void)
{
    ...
    case MOTOR_RAMPUP_STATE:
        task_set_state(motor_rampup, RUNNABLE);
        break;
    case MOTOR_RAMPDOWN_STATE:
        task_set_state(motor_rampdown, RUNNABLE);
        break;
    default:
        break;
    }
...
static void motor_hold(void)
{
    task_set_state(motor_rampup, PAUSED);
    task_set_state(motor_rampdown, PAUSED);
}
...
{
    task_set_state(motor_toggle_on_off, PAUSED);
...
static void motor_initiate_startup(void)
{
    ...
    task_set_state(motor_startup, RUNNABLE);
}
...
void motor_startup(void)
{
    OCR1A += 1;
    if (OCR1A >= MOTOR_MIN_SPEED) {
        task_set_state(motor_startup, PAUSED);
...
static void motor_initiate_shutdown(void)
{
    ...
    task_set_state(motor_rampup, PAUSED);
    task_set_state(motor_rampdown, PAUSED);
    task_set_state(motor_shutdown, RUNNABLE);
}
...
void motor_shutdown(void)
{
    OCR1A -= 1;
    if (OCR1A == 0) {
        task_set_state(motor_shutdown, PAUSED);
        motor_is_shutting_down_or_starting_up = 0;
        motor_is_on                           = 0;
    }
}
